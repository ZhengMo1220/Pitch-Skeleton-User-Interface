...
from camera_objects.single_camera import FlirCameraSystem
...
class DualFlirSystem(TwoCamerasSystem):
    ...
    def release(self) -> bool:
        ...
        if getattr(self, 'camera1', None) is not None:
            try:
                release_ok = self.camera1.release() and release_ok
            except Exception:
                release_ok = False
            finally:
                self.camera1 = None

        if getattr(self, 'camera2', None) is not None:
            try:
                release_ok = self.camera2.release() and release_ok
            except Exception:
                release_ok = False
            finally:
                self.camera2 = None
