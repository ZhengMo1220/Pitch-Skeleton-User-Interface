...
class FlirCameraSystem(SingleCameraSystem):
    ...
    def __init__(self,
        ...
        FlirCameraSystem._active_instances += 1
    ...
    def get_grayscale_image(self) -> Tuple[bool, np.ndarray]:
        ...
        try:
            ...
            logging.error('SN %s: Failed to get image: %s', serial_number, str(e))
            # Try to recover by clearing the buffer
            try:
                if self.cam.IsStreaming():
                    self.cam.EndAcquisition()
                    self.cam.BeginAcquisition()
                    logging.info('SN %s: Restarted acquisition after error', serial_number)
            except Exception as recovery_error:
                logging.error('SN %s: Failed to recover: %s', serial_number, str(recovery_error))
            return False, None
        ...
        # Release image resources to prevent memory leaks
        image_result.Release()
    ...
    def get_fps(self) -> int:
        ...
        returns:
        int:
            - int: Fps of the camera system.
        """
        return int(self.full_config['acquisition_settings']['fps'])
    def release(self) -> bool:
        ...
        if cam is not None:
            ...
            try:
                ...
                released_ok = False
                logging.warning("EndAcquisition failed for camera %s: %s", self.serial_number, str(e))
            ...
            try:
                ...
                released_ok = False
                logging.warning("DeInit failed for camera %s: %s", self.serial_number, str(e))
        ...
        if cam_list is not None:
            ...
            try:
                ...
                released_ok = False
                logging.warning("Clear camera list failed for camera %s: %s", self.serial_number, str(e))
        self.cam_list = None
        if cam_list is not None:
            del cam_list
        ...
        if system is not None:
            if FlirCameraSystem._active_instances > 0:
                FlirCameraSystem._active_instances -= 1
            ...
            if FlirCameraSystem._active_instances == 0:
                ...
                try:
                    ...
                    released_ok = False
                    logging.error("ReleaseInstance failed for camera %s: %s", self.serial_number, str(e))
                else:
                    logging.info("System released.")
            else:
                logging.info(
                    "Skipping system release for camera %s (%d FLIR camera instance(s) still active).",
                    self.serial_number,
                    FlirCameraSystem._active_instances
                )
        self.system = None
    ...
    def update_white_balance(self, red_ratio: float = None, blue_ratio: float = None, gain: float = None):
        """
        Update white balance settings and apply to camera immediately.

        Args:
            red_ratio (float, optional): New red ratio.
            blue_ratio (float, optional): New blue ratio.
            auto (str, optional): 'Off', 'Once', or 'Continuous'.
        """
        wb = self.full_config['white_balance_settings']
        gs = self.full_config['gain_settings']
        if red_ratio is not None:
            wb['white_balance_red_ratio'] = red_ratio
        if blue_ratio is not None:
            wb['white_balance_blue_ratio'] = blue_ratio
        if gain is not None:
            gs['gain_value'] = gain
        self._configure_white_balance()
        self._configure_gain()
