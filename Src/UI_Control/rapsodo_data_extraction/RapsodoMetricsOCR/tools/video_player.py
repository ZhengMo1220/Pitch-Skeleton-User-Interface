"""
tools/video_player.py — Rapsodo Video Player: Offline Data Extraction Tool

Process pre-recorded Rapsodo iPad screen videos to extract hitting or pitching
metrics. Mode is selected via a dialog at startup. No CLI arguments required;
video file is selected through the UI.
...
# HitCounterReader.read() call does not re-assign this attribute. The
# mode-specific readers in modes/hitting.py go through the same pytesseract
# module, so a single assignment here is enough for the whole tool.
# (Commit 3b — Fix 3 hot-path cleanup.)
...
class InvalidDetector:
    ...
    Uses OpenCV pixel analysis instead of Tesseract:
      1. Crop each ROI (hitting or pitching, depending on mode)
    ...
    This correctly handles both '---' and single '-' states without Tesseract,
    and is immune to OCR parse failures on dash characters.
    """

    def __init__(self, mode: str = "pitching") -> None:
...
class HitCounterReader:
    """
    Read the integer hit counter displayed on the left side of the Rapsodo UI.

    Uses the same adaptive-threshold pipeline as BaseOCRReader but returns an
    int (or None on parse failure).  The counter ROI is defined in config as
    HIT_COUNTER_ROI = (x, y, w, h) as fractions of the raw frame dimensions
    (not the warped screen crop — the counter is visible in the full frame).

    Fix 2 — PSM voting: for each (block_size, C) attempt, the same binarised
    crop is passed to every PSM in config.OCR_PSM_FALLBACK_MODES and the
    parsed integers are tallied. A value is only accepted when at least
    config.HIT_COUNTER_VOTE_MIN_AGREEMENT PSMs agree. This was added after
    run_log.txt analysis showed individual PSMs misreading single digits
    (e.g. 3→5, 4→44) and permanently poisoning VideoProcessor._last_hit_count.
    """

    # Known Tesseract misread pair: 3 ↔ 5 on Rapsodo counter font.
    _AMBIGUOUS_DIGITS = frozenset({3, 5})

    def read(self, frame: np.ndarray) -> int | None:
        """
        Return the current hit counter integer, or None on failure.

        Fix 16: two-phase architecture.
        Fix 17: Phase 1 skips known ambiguous values (3/5) and falls
        through to Phase 2 voting for disambiguation.
        """
        h, w = frame.shape[:2]
        x, y, rw, rh = config.HIT_COUNTER_ROI
        ...
        # ── Phase 1: single-shot fast read ───────────────────────────
        # Fix 17: if result is 3 or 5, fall through to Phase 2 because
        # Tesseract systematically confuses these on the Rapsodo counter font.
        ...
        fast_cfg = config.build_tesseract_config(7)
        ...
        if match:
            ...
            if 0 <= value <= 999 and value not in self._AMBIGUOUS_DIGITS:
                return value

        # ── Phase 2: full (blk,C) × PSM voting sweep ────────────────
        for blk, C in config.OCR_ADAPTIVE_ATTEMPTS:
            ...
            for psm in config.OCR_PSM_FALLBACK_MODES:
                ...
                if votes[value] >= config.HIT_COUNTER_VOTE_MIN_AGREEMENT:
                    winner = value
                    break
...
class PitchCounterReader:
    ...
    # Known Tesseract misread pair (same as HitCounterReader).
    _AMBIGUOUS_DIGITS = frozenset({3, 5})
...
class OCRTask:
    ...
    ``kind`` values:
      * ``"counter"`` — run HitCounterReader on ``frame``. On an accepted
        +1 increment, the worker calls ``_schedule_trigger`` to request a
        follow-up "trigger" task NEW_HIT_WAIT_MS later.
      * ``"trigger"`` — run HittingOCRReader + InvalidDetector on ``frame``
...
class OCRWorker(QThread):
    ...
    new_hit = pyqtSignal(object, int)   # HitMetrics or PitchMetrics, hit_id
    ...
    def __init__(
        ...
        self.invalid_detector = InvalidDetector(mode="pitching")
    ...
    def _handle_counter_task(self, task: OCRTask) -> None:
        ...
        count = self.counter_reader.read(task.frame)
        ...
        if accepted > previous:
            ...
            # future. In hitting mode jumps are always +1 (MAX_JUMP=1). In
            # pitching mode jumps up to +3 are accepted because the counter
            # OCR is less stable — intermediate reads get missed.  Regardless
            # of the jump size, only ONE trigger is scheduled (for the latest
            # frame) because the metrics screen already shows the most recent
            # entry; past entries that were missed cannot be recovered.
            self._last_hit_count = accepted
    ...
    def _handle_trigger_task(self, task: OCRTask) -> None:
        ...
        if is_invalid:
            ...
            if self._mode == "pitching":
                logger.debug(
                    "frame %d — NEW PITCH vel=%s spin=%s dir=%s eff=%s",
                    task.frame_index,
    ...
    def _vote_metrics(
        ...
        wall time roughly 3× (3× 50 s → ~17 s observed baseline on hitting
        V1). Thread-safety: the readers themselves hold no mutable per-call
    ...
    def _accept_counter_reading(
        ...
        """
        Sanity-filter a raw HitCounterReader value before it is allowed to
        ...
        Rejects (returns None) when:
          ...
          * ``count`` exceeds the baseline by more than
            ``config.HIT_COUNTER_MAX_JUMP`` (outlier — almost certainly an
            OCR misread such as "4" → "44" observed in run_log.txt).

        A rejected reading is logged but leaves ``_last_hit_count``
        unchanged, which is the whole point of Fix 2: without this guard, a
        single misread can poison the baseline for the remainder of the
        video.
        """
        if count is None:
            return None
        ...
        max_jump = (
            config.PITCH_COUNTER_MAX_JUMP
            if self._mode == "pitching"
            else config.HIT_COUNTER_MAX_JUMP
        )
...
class VideoProcessor(QThread):
    ...
    def _wait_for_stability(
        ...
        roi_source = (
            config.PITCHING_ROI
            if self.worker._mode == "pitching"
            else config.HITTING_ROI
        )
        rois = list(roi_source.values())
...
class VideoPlayerWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        self._ev_label = QLabel("EXIT VELOCITY\n—")
        self._la_label = QLabel("LAUNCH ANGLE\n—")
        self._dist_label = QLabel("DISTANCE\n—")
        self._eff_label = QLabel("")
        self._eff_label.setVisible(False)
        for lbl in (self._ev_label, self._la_label, self._dist_label, self._eff_label):
    ...
    def _on_open_clicked(self) -> None:
        # Pitching-only delivery build: skip mode selection.
        selected_mode = "pitching"
        ...
        self._start_processing(file_path, player_name, selected_mode)

    # ------------------------------------------------------------------
    # Processing lifecycle
    # ------------------------------------------------------------------

    def _start_processing(self, video_path: str, player_name: str, mode: str = "pitching") -> None:
        ...
        self._setup_data_labels(mode)

        self._session_manager = SessionManager()
        video_filename = Path(video_path).name
        output_path = (
            config.OUTPUT_JSON_PITCHING if mode == "pitching"
            else config.OUTPUT_JSON_HITTING
        )
    ...
    def on_new_hit(self, metrics, hit_id: int) -> None:
        ...
        vel = f"{metrics.velocity_mph:.1f} MPH" if metrics.velocity_mph is not None else "—"
        ...
        item = QListWidgetItem(f"#{hit_id}  {vel}  {spin}  {sdir}  {eff}")
    ...
    def _setup_data_labels(self, mode: str = "pitching") -> None:
...
class _VideoDataWriter(DataWriter):
    ...
    def __init__(
        ...
        super().__init__(output_path=output_path, mode=mode, session_id=session_id)
    ...
    def write_invalid_entry(self, player: str) -> int:
        ...
        if self._mode == "pitching":
            base["velocity_mph"] = None
            base["total_spin_rpm"] = None
            base["spin_direction"] = None
            base["spin_efficiency_pct"] = None
        else:
            base["exit_velocity_mph"] = None
            base["launch_angle_deg"] = None
            base["distance_ft"] = None
