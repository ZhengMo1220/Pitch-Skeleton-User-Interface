...
class PitchingOCRReader(BaseOCRReader):
    ...
    # Only the three NUMERIC fields go into ROI_DEFINITIONS / VALIDATION_RANGES
    # because BaseOCRReader._read_field() assumes a numeric validation range.
    ...
    ROI_DEFINITIONS: dict[str, RelativeROI] = {
        ...
        for name in ("velocity_mph", "total_spin_rpm", "spin_efficiency_pct",
                     "v_break_in", "h_break_in",
                     )
    }
    VALIDATION_RANGES: dict[str, tuple[float, float]] = config.PITCHING_VALIDATION

    # Override Fix 4d frac60_min3 to frac70_min4. Pitching's italic-bold
    # digits produce more noisy PSM ties than hitting's font: leading
    # '7'/'8'/'9' regularly get eaten by PSM 11/4 under some (block, C)
    # thresholds, so a 60 % winner can still carry a silent-wrong read
    # past the vote. Requiring 70 % of ≥4 total weighted votes forces
    # those marginal wins to fall through to null. Validated against
    # the 18-entry pitcher sample (see CHANGELOG Fix 10 — hitting mode
    # is unaffected because it keeps BaseOCRReader's default).
    ...
    def _read_easyocr(
        ...
        # Choose allowlist based on field type
        if field_name == "total_spin_rpm":
            allowlist = "0123456789"
        else:
            allowlist = "0123456789."

        results = reader.readtext(gray_up, allowlist=allowlist, detail=1)
        if not results:
            logger.debug("EasyOCR %s: no results", field_name)
            return None

        lo, hi = config.PITCHING_VALIDATION.get(
