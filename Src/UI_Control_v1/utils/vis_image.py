...
class ImageDrawer():
    ...
    def draw_styled_metric(self, img, point, label, value, unit="", direction='right'):
        ...
        # 2. 計算引導線路徑
        offset_x = 100 if direction == 'right' else -100
        offset_y = -60
        end_point = (x + offset_x, y + offset_y)
        text_line_end = (end_point[0] + (80 if direction == 'right' else -80), end_point[1])
        ...
        # 繪製主數值 (加粗效果)
        draw.text((tx, ty + 5), val_text, font=font_val, fill=(255, 255, 255), 
                  stroke_width=2, stroke_fill=(255, 255, 255)) 
        
        # 繪製中文標籤 (微加粗)
        draw.text((tx, ty + 55), label_text, font=font_label, fill=(220, 220, 220),
                  stroke_width=1, stroke_fill=(220, 220, 220))

        return cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)

    def drawAngleInfo(self, img: np.ndarray, frame_num: int, keyframe_type: str = None) -> np.ndarray:
        """
        根據關鍵幀類型繪製不同的分析信息。
    ...
    def _drawKeyframeFootContact(self, img: np.ndarray, frame_num: int) -> np.ndarray:
        """
        ...
        try:
            ...
            try:
                ...
                if shoulder_hip_angle is not None and keypoints is not None:
                    ...
                    img = self.draw_styled_metric(img, hip_center, "肩髖分離角度", shoulder_hip_angle, "°", direction='left')
            ...
            try:
                ...
                if stride_distance is not None and keypoints is not None:
                    ...
                    img = self.draw_styled_metric(img, left_ankle, "跨步距離", f"{stride_distance:.2f}", "m", direction='right')
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if shoulder_angle is not None and keypoints is not None:
                    ...
                    img = self.draw_styled_metric(img, keypoints[6], "最大肩外旋角度", shoulder_angle, "°", direction='right')
            except Exception as e:
    ...
    def _drawKeyframeRelease(self, img: np.ndarray, frame_num: int) -> np.ndarray:
        """
        繪製第三個關鍵幀的信息：最大手腕速度
        """
        try:
            ...
            try:
                ...
                if wrist_speed is not None and keypoints is not None:
                    ...
                    img = self.draw_styled_metric(img, keypoints[10], "手腕速度", f"{wrist_speed:.2f}", "m/s", direction='left')
            except Exception as e:
            ...
            try:
                ...
                if extension_distance is not None and keypoints is not None:
                    img = self.draw_styled_metric(img, keypoints[10], "出手點距離", f"{extension_distance:.2f}", "m", direction='right')
            except Exception as e:
                print(f"Error calculating stride distance: {e}")
    ...
    def _calculate_stride_distance(self, frame_num: int) -> float:
        ...
        try:
            ...
            # 計算兩點之間的歐幾里得距離
            distance = np.linalg.norm(np.array(left_ankle) - np.array(right_ankle)) * 19.92586
            return distance
        except Exception as e:
            print(f"Error calculating stride distance: {e}")
            return None
    ...
    def _calculate_extension_distance(self, frame_num: int) -> float:
        ...
        try:
            ...
            distance_x = abs(float(right_wrist[0]) - float(right_ankle[0])) * 19.92586
            return distance_x
        except Exception as e:
            print(f"Error calculating stride distance: {e}")
            return None
