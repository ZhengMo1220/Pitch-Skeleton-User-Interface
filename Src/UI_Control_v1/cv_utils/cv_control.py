...
class Camera:
    def __init__(self, camera_idx:int = 0):
        ...
        # 預存最近12幀（用於自動錄影的前幀）
        self.pre_frames = deque(maxlen=60)
    ...
    def startRecording(self, filename: str, filename_2: str):
        ...
        self.video_writer = VideoWriterThread(filename, filename_2, frame_width, frame_height, fps=fps//3)
    ...
    def stop_auto_recording(self):
        ...
        return frames, actual_fps # 回傳計算出來的真實 FPS
...
class VideoLoader:
    ...
    def getVideoImage(self, frame_num:int) : 
        return self.video_frames[frame_num].copy(), self.video_frames_2[frame_num].copy()
    ...
    def saveVideo(self, output_folder:str = None):
        ...
        save_person_df_2 = self.image_drawer_2.pose_estimater.person_df
        ...
        video_writer_2 = cv2.VideoWriter(save_location_2, cv2.VideoWriter_fourcc(*'mp4v'), self.video_fps, self.video_size)
